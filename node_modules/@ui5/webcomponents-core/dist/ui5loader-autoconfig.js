(function () {
  "use strict";
  var ui5loader = window.sap && window.sap.ui && window.sap.ui.loader, oCfg = window["sap-ui-config"] || ({}), sBaseUrl, bNojQuery, aScripts, rBootScripts, i, oBootstrapScript, sBootstrapUrl, bExposeAsAMDLoader = false;
  function findBaseUrl(oScript, rUrlPattern) {
    var sUrl = oScript && oScript.getAttribute("src"), oMatch = rUrlPattern.exec(sUrl);
    if (oMatch) {
      sBaseUrl = oMatch[1] || "";
  ...
  if (typeof oCfg === "object" && typeof oCfg.resourceRoots === "object" && typeof oCfg.resourceRoots[""] === "string") {
  ...
  (function () {
    var mUrlMatch = (/(?:^|\?|&)sap-ui-debug=([^&]*)(?:&|$)/).exec(window.location.search), vDebugInfo = mUrlMatch && decodeURIComponent(mUrlMatch[1]);
    try {
    ...
    var fnIgnorePreload;
    if (typeof vDebugInfo === "string") {
      var sPattern = "^(?:" + vDebugInfo.split(/,/).map(makeRegExp).join("|") + ")", rFilter = new RegExp(sPattern);
