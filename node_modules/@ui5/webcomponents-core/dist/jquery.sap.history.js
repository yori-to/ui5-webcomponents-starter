...
import escapeRegExp from './sap/base/strings/escapeRegExp.js';
(function (window) {
    ...
    function calculateStepsToHash(sCurrentHash, sToHash, bPrefix) {
        var iCurrentIndex = jQuery.inArray(sCurrentHash, hashHistory), iToIndex, i, tempHash;
        if (iCurrentIndex > 0) {
            if (bPrefix) {
    ...
    function getAppendId(sHash) {
        var iIndex = jQuery.inArray(currentHash, hashHistory), i, sHistory;
        if (iIndex > -1) {
    ...
    function calcStepsToRealHistory(sCurrentHash, bForward) {
        var iIndex = jQuery.inArray(sCurrentHash, hashHistory), i;
        if (iIndex !== -1) {
            if (bForward) {
        ...
        if (sHash.length === 0) {
            ...
            if (isVirtualHash(sHash)) {
                ...
                if (oParsedHash) {
                    ...
                    if (oRoute) {
                        oRoute.action.apply(null, [
                            oParsedHash.oStateData,
                            sNavType
                        ]);
                    }
                } else {
