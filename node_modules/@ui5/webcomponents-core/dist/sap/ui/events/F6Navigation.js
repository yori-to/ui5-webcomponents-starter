...
F6Navigation.handleF6GroupNavigation = function (oEvent, oSettings) {
    ...
    function navigate(oSource, aScopes, bForward) {
        ...
        if ($Target && $Target.length) {
            ...
            if (oCustomGroup && oCustomGroup.id) {
                var oControl = sap.ui.getWCCore().byId(oCustomGroup.id);
                if (oControl) {
    ...
    if (oEvent.type != 'keydown' || oEvent.key != 'F6' || oEvent.isMarked('sapui5_handledF6GroupNavigation') || oEvent.isMarked() || oEvent.isDefaultPrevented()) {
        return;
    }
