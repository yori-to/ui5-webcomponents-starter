...
import ElementMetadata from '../core/ElementMetadata.js';
...
var PropertyList = EventProvider.extend('sap.ui.debug.PropertyList', {
...
PropertyList.prototype.update = function (oParams) {
    ...
    this.mProperties = {};
    var oMetadata = oControl.getMetadata(), aHTML = [];
...
PropertyList.prototype.getAggregationsAsProperties = function (oMetadata) {
    ...
    for (var sAggrName in oMetadata.getAggregations()) {
        ...
        if (oAggr.altTypes && oAggr.altTypes[0] && isSimpleType(oAggr.altTypes[0])) {
            oResult[sAggrName] = {
                name: sAggrName,
                type: oAggr.altTypes[0],
                _oParent: oAggr._oParent
            };
        }
    }
    return oResult;
...
PropertyList.prototype.printProperties = function (aHTML, oControl, mProperties, bAggregation) {
    for (var i in mProperties) {
        var sName = i, sType = mProperties[i].type, oMethod = oControl['get' + sName];
        if (!oMethod) {
...
PropertyList.prototype.applyChanges = function (sId) {
    var oSource = this.oParentDomRef.ownerDocument.getElementById(sId), sControlId = oSource.getAttribute('sap-id'), oControl = this.oCore.byId(sControlId), aInput = oSource.parentNode.getElementsByTagName('INPUT'), aSelect = oSource.parentNode.getElementsByTagName('SELECT'), oMethod;
    for (var i = 0; i < aInput.length; i++) {
        var oInput = aInput[i], sName = oInput.getAttribute('sap-name');
    ...
    for (var i = 0; i < aSelect.length; i++) {
        var oSelect = aSelect[i], sName = oSelect.getAttribute('sap-name');
        oMethod = oControl['set' + sName];
        if (!oMethod) {
            sName = capitalize(sName, 0);
        }
