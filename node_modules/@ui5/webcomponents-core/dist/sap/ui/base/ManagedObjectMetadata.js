...
Property.prototype.generate = function (add) {
    ...
    add(that._sGetter, function () {
        return this.getProperty(n);
    });
    add(that._sMutator, function (v) {
        this.setProperty(n, v);
        return this;
    }, that);
    if (that.bindable) {
        add(that._sBind, function (p, fn, m) {
            this.bindProperty(n, p, fn, m);
            return this;
        }, that);
        add(that._sUnbind, function (p) {
...
Aggregation.prototype.generate = function (add) {
    ...
    if (!that.multiple) {
        ...
        add(that._sMutator, function (a) {
            this.addAggregation(n, a);
            return this;
        }, that);
        add(that._sInsertMutator, function (i, a) {
            this.insertAggregation(n, i, a);
            return this;
        }, that);
        add(that._sRemoveMutator, function (a) {
            return this.removeAggregation(n, a);
        });
        add(that._sRemoveAllMutator, function () {
            return this.removeAllAggregation(n);
        });
        add(that._sIndexGetter, function (a) {
            return this.indexOfAggregation(n, a);
        });
    }
    add(that._sDestructor, function () {
        this.destroyAggregation(n);
        return this;
    });
    if (that.bindable) {
        add(that._sBind, function (p, t, s, f) {
...
function AggregationForwarder(oAggregation) {
    ...
    if (oForwardTo.getter) {
        ...
        this._getTarget = function (sIdSuffix) {
            return function () {
                return sap.ui.getWCCore().byId(this.getId() + sIdSuffix);
            };
        }(oForwardTo.idSuffix);
    } else {
...
Event.prototype.generate = function (add) {
    ...
    add(that._sMutator, function (d, f, o) {
        this.attachEvent(n, d, f, o);
        return this;
    }, that);
    add(that._sDetachMutator, function (f, o) {
        this.detachEvent(n, f, o);
        return this;
    });
    add(that._sTrigger, function (p) {
        return this.fireEvent(n, p, allowPreventDefault, enableEventBubbling);
    });
};
...
function preloadDesigntimeLibrary(oMetadata) {
    var sLibrary = oMetadata.getLibraryName(), sPreload = sap.ui.getWCCore().getConfiguration().getPreload(), oLibrary = sap.ui.getWCCore().getLoadedLibraries()[sLibrary];
...
function mergeDesignTime(mOwnDesignTime, mParentDesignTime, sScopeKey) {
    return merge({}, getScopeBasedDesignTime(mParentDesignTime, sScopeKey), { templates: { create: null } }, getScopeBasedDesignTime(mOwnDesignTime, sScopeKey), {
...
function uid(sId) {
    ...
    sId = (sUIDPrefix || (sUIDPrefix = sap.ui.getWCCore().getConfiguration().getUIDPrefix())) + sId;
...
ManagedObjectMetadata.isGeneratedId = function (sId) {
    sUIDPrefix = sUIDPrefix || sap.ui.getWCCore().getConfiguration().getUIDPrefix();
    rGeneratedUID = rGeneratedUID || new RegExp('(^|-{1,3})' + escapeRegExp(sUIDPrefix));
    return rGeneratedUID.test(sId);
};
export default ManagedObjectMetadata;